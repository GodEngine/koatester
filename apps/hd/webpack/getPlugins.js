...
export default ({
  publicPath, sourcePath, distPath, project, nodeEnv, vendorExtractTextPlugin,
  extractTextPlugin, needSprite, needRetina, isProd
} = {}) => {
  const plugins = [
    // 清除上次打包生成的文件，保证目录干净
      ...
      .map(dir => {
        ...
        const spritesmithOpts = {
          ...
          target: {
            ...
            // 生成用来引用的 scss 文件
            css: [
              [path.resolve(sourcePath, `scss/_sprite${suffix}.scss`), {
                format: 'hbs'
              }]
            ]
          },
